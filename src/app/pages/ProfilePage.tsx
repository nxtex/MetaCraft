...
import { User, Mail, Lock, Bell, Shield, Palette, Database, Award, TrendingUp, Loader2, CheckCircle } from 'lucide-react';
...
export function ProfilePage() {
  ...
  useEffect(() => {
    filesApi.history().then(data => setFileCount(data.length)).catch(() => {});
  }, []);

  async function handleSaveProfile(e: React.FormEvent) {
    e.preventDefault();
    setSavingProfile(true);
    // Profile update endpoint not yet implemented; optimistic UI only
  ...
  const stats = [
    ...
    { label: 'Jours depuis inscription', value: user ? String(Math.floor((Date.now() - new Date(user.created_at).getTime()) / 86400000)) : '…', icon: TrendingUp, color: '#2AFC98' },
    { label: 'Niveau fouilleur', value: (fileCount ?? 0) >= 50 ? 'Expert' : (fileCount ?? 0) >= 10 ? 'Intermédiaire' : 'Débutant', icon: Award, color: '#E8732A' },
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            <StratumCard className="p-6">
              <motion.div className="w-24 h-24 sm:w-32 sm:h-32 mx-auto mb-4 rounded-full flex items-center justify-center relative overflow-hidden"
                style={{ background: 'linear-gradient(135deg, #C9A84C, #E8732A)' }}
                whileHover={{ scale: 1.05 }}>
                <User className="w-12 h-12 sm:w-16 sm:h-16" style={{ color: '#080A0F' }} />
              </motion.div>

              <h3 className="text-xl text-center mb-1" style={{ fontFamily: 'Cinzel, serif', color: '#EDE8DC' }}>{user?.name}</h3>
              <p className="text-sm text-center mb-4" style={{ fontFamily: 'IBM Plex Mono, monospace', color: '#7A7060' }}>{user?.email}</p>

              <div className="px-3 py-2 text-center text-xs mb-4"
                style={{ fontFamily: 'Bebas Neue, cursive', letterSpacing: '2px',
                  background: 'linear-gradient(90deg, rgba(201,168,76,0.1), rgba(232,115,42,0.1))', color: '#C9A84C' }}>
                ⭐ {stats[2].value.toUpperCase()}
              </div>
                ...
                {(['profile', 'settings', 'stats'] as const).map(tab => (
                  <motion.button key={tab} onClick={() => setActiveTab(tab)}
                    ...
                    style={{
                      ...
                      fontFamily: 'Bebas Neue, cursive', letterSpacing: '2px',
              ...
              <motion.button onClick={logout} className="w-full mt-6 py-2 text-sm"
                style={{ border: '1px solid rgba(192,57,43,0.3)', fontFamily: 'Bebas Neue, cursive', letterSpacing: '2px', color: '#C0392B' }}
                whileHover={{ backgroundColor: 'rgba(192,57,43,0.1)' }}>
                DÉCONNEXION
              </motion.button>
            </StratumCard>
            <div className="hidden lg:block"><GoogleAdSpace slot="sidebar-1" format="vertical" /></div>
          </div>

          {/* Main */}
          <div className="lg:col-span-9 space-y-6">
            <div className="flex justify-center"><GoogleAdSpace slot="header-1" format="horizontal" /></div>
            ...
            {activeTab === 'profile' && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <StratumCard className="p-4 sm:p-6">
                  ...
                  {profileSaved && (
                    <div className="flex items-center gap-2 p-3 mb-4"
                      ...
                      <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '13px', color: '#2AFC98' }}>Profil mis à jour</p>
                    </div>
                  )}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="email" value={user?.email ?? ''} disabled
                            className="w-full pl-10 pr-4 py-3 outline-none opacity-60"
                            style={{ backgroundColor: '#141C2A', border: '1px solid rgba(201,168,76,0.15)', fontFamily: 'JetBrains Mono, monospace', color: '#EDE8DC' }} />
                        </div>
                      </div>
                    </div>
                    <motion.button type="submit" disabled={savingProfile}
                      className="mt-6 w-full sm:w-auto px-8 py-3 flex items-center gap-2"
                ...
                <div className="flex justify-center"><GoogleAdSpace slot="content-1" format="rectangle" /></div>
              </motion.div>
            )}
                ...
                <StratumCard className="p-4 sm:p-6">
                  ...
                  <div className="space-y-8">
                    <div>
                      ...
                      <motion.button className="w-full px-4 py-3 text-left flex items-center justify-between"
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {['Sombre', 'Clair', 'Auto'].map(theme => (
                          <motion.button key={theme} className="p-3 sm:p-4 text-center text-sm"
                            style={{
                            ...
                            whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>{theme}
                          </motion.button>
                        ))}
                      </div>
                    </div>
            ...
            {activeTab === 'stats' && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                      ...
                      <StratumCard hover className="p-4 sm:p-6">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-4xl sm:text-5xl" style={{ fontFamily: 'Cinzel, serif', color: stat.color }}>{stat.value}</p>
                          </div>
                ...
                <div className="flex justify-center"><GoogleAdSpace slot="stats-1" format="horizontal" /></div>
              </motion.div>
            )}
          </div>
        </div>
